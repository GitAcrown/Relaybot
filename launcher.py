...
def parse_cli_arguments():
    parser = argparse.ArgumentParser(description="Relay - Discord Bot")
    parser.add_argument("--start", "-s",
                        help="Starts Relay",
                        action="store_true")
    parser.add_argument("--auto-restart",
                        help="Autorestarts Relay in case of issues",
                        action="store_true")
    parser.add_argument("--update-red",
                        help="Updates Relay (git)",
                        action="store_true")
...
def update_menu():
    ...
    while True:
        ...
        print("Update:\n")
        print("Relay:")
        print("1. Update Relay + requirements (recommended)")
        print("2. Update Relay")
...
def maintenance_menu():
    ...
    while True:
        print(INTRO)
        print("Maintenance:\n")
        print("1. Repair Relay (discards code changes, keeps data intact)")
        ...
        if choice == "1":
            ...
            print("Are you sure? This will wipe ALL your Relay's installation "
...
def run_red(autorestart):
    ...
    if verify_requirements() is None:
        print("You don't have the requirements to start Relay. "
    ...
    while True:
        try:
            ...
            if code == 0:
                ...
                print("Restarting Relay...")
                continue
            else:
                if not autorestart:
                    break

    print("Relay has been terminated. Exit code: %d" % code)
...
def create_fast_start_scripts():
    """Creates scripts for fast boot of Relay without going
...
def main():
    ...
    if IS_WINDOWS:
        os.system("TITLE Relay Discord Bot - Launcher")
    clear_screen()
    ...
    while True:
        ...
        if not is_git_installation:
            print("WARNING: It doesn't look like Relay has been "
        ...
        print("1. Run Relay /w autorestart in case of issues")
        print("2. Run Relay")
        print("3. Update")
...
if __name__ == '__main__':
    ...
    os.chdir(dirname)
    if not PYTHON_OK:
        print("Relay needs Python 3.5 or superior. Install the required "
              "version.\nPress enter to continue.")
        if INTERACTIVE_MODE:
            wait()
        exit(1)
    if pip is None:
        print("Relay cannot work without the pip module. Please make sure to "
